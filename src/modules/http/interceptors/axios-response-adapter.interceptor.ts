...
import type { AxiosLikeResponse, AxiosLikeRequestConfig } from '../interfaces/axios-compatible.interface';
...
export class AxiosResponseAdapterInterceptor implements HttpInterceptor {
  intercept(
    ...
    return next.handle(request).pipe(
      mergeMap(async (response: Dispatcher.ResponseData) => {
        ...
        try {
          if (response.body) {
            if (contentType.includes('application/json')) {
            ...
            } else if ((response.statusCode === 204 || response.statusCode === 304) && !contentType) {
              ...
              parsedData = Buffer.from(await response.body.arrayBuffer());
            }
          } else {
        ...
        const axiosLikeResponse: AxiosLikeResponse = {
          ...
          statusText: STATUS_TEXT_MAP[response.statusCode] || 'Unknown',
        ...
        const validateStatus = config.validateStatus || ((status: number) => {
          // Default axios behavior: only 2xx are valid
          return status >= 200 && status < 300;
        });
        const isValidStatus = validateStatus(response.statusCode);
 ...
 */
export const axiosResponseAdapter: HttpInterceptorFunction = (request, next) => {
